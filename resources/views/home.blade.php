  ...
  @auth
    @if (is_null(auth()->user()->email_verified_at))
      <div x-show="verifyModal" x-cloak x-transition.opacity
        ...
        <div @click.outside="verifyModal = false"
          class="bg-[#fffdf7]/95 w-full max-w-lg mx-4 rounded-[28px]
                                                                       shadow-[0_18px_55px_rgba(190,143,43,0.35)]
                                                                       border border-[#f4ddab] px-6 py-7 md:px-8 md:py-8 relative">

          {{-- Tombol close kecil --}}
          <button type="button"
            class="absolute top-3 right-3 w-8 h-8 rounded-full bg-slate-100 hover:bg-slate-200
    ...
    <div class="backdrop-blur-md bg-white/80 border-b border-amber-100/60 shadow-sm">
      <div class="max-w-7xl mx-auto px-5 md:px-8 py-3.5 flex items-center justify-between gap-4">
        ...
        <nav class="hidden md:flex items-center gap-6 text-[14px] font-medium text-amber-900/80">
          ...
          {{-- SELALU ADA, BAIK LOGIN MAUPUN TIDAK --}}
        ...
        <div class="flex items-center gap-3">
          ...
          @auth
            @php
              $user = auth()->user();
              $avatar = $user->avatar
                ? asset($user->avatar)          // langsung pakai path dari DB
                : asset('default-avatar.png');  // default kalau belum ada foto
            @endphp
          ...
          <div class="flex w-[400%] animate-gallery">
            ...
            @if ($itemsBaju->isNotEmpty())
              @foreach ($itemsBaju as $item)
                <img src="{{ asset($item->image_path) }}" class="w-1/4 h-[320px] object-cover" alt="{{ $item->judul }}">
  ...
  <section id="tim" class="bg-white/70 border-y border-amber-100/60 py-14 scroll-mt-24 md:scroll-mt-32">
    <div class="max-w-6xl mx-auto px-6 md:px-10">
      ...
      @php
        ...
        // AMBIL FOTO DARI DB SESUAI URUTAN
        $teamFromDb = $team ?? collect();

        $teamToShow = collect();

        foreach ($fixedMembers as $index => $fixed) {
          $photo = null;

          if (isset($teamFromDb[$index]) && !empty($teamFromDb[$index]->photo)) {
            $photo = $teamFromDb[$index]->photo;
          }

          $teamToShow->push((object) [
            'name' => $fixed['name'],
            'role' => $fixed['role'],
            'division' => $fixed['division'],
            'photo' => $photo,
          ]);
        }
      @endphp
        ...
        @foreach ($teamToShow as $index => $member)
          @php
            ...
            $photoUrl = $member->photo
              ? asset($member->photo)   // langsung pakai path dari DB
  ...
  @auth
      ...
      <div @click.outside="profileModal=false"
        ...
        <div class="flex justify-end mt-6 gap-3">
          ...
          <button type="button" @click="profileModal=false; editModal=true"
            class="px-5 py-2 rounded-lg text-sm text-white shadow-md
                                                                                                     bg-gradient-to-r from-[#f7e07b] via-[#eab308] to-[#c98a00]
    ...
    <div x-show="editModal" x-cloak x-transition.opacity
      ...
      <div @click.outside="editModal = false"
        class="bg-white rounded-[32px] shadow-2xl border border-amber-100
        ...
        <form action="{{ route('profile.update') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
          ...
          <div class="flex justify-end gap-3 pt-4">
            <button type="button" @click="editModal = false"
              class="px-5 py-2.5 rounded-xl bg-slate-100 text-slate-700 text-sm md:text-base
                                                                                                       hover:bg-slate-200 transition">
              Batal
            </button>

            <button type="submit"
              class="px-6 py-2.5 rounded-xl text-sm md:text-base text-white font-semibold shadow-md
                                                                                                       bg-gradient-to-r from-[#f7e07b] via-[#eab308] to-[#c98a00]
                                                                                                       hover:opacity-90 transition">
              Simpan Perubahan
            </button>
