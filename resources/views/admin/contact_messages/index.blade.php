...
@section('content')
    ...
    <div class="px-4 sm:px-6 lg:px-8 py-6" x-data="{
        ...
        <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between mb-6">
            <div>
                <h1 class="text-2xl md:text-3xl font-bold text-[#c98a00] mb-1">
                    Pesan Masuk
                </h1>
            ...
            <div class="flex flex-col xs:flex-row gap-3">
                {{-- TOTAL PESAN --}}
                <div class="bg-white rounded-2xl shadow border border-amber-100 px-4 py-3 min-w-[120px]">
                ...
                {{-- BELUM DIBACA --}}
                <div class="bg-[#fff7cf] rounded-2xl shadow border border-amber-200 px-4 py-3 min-w-[120px]">
                    <p class="text-[11px] uppercase tracking-wide text-slate-500">
                        Belum Dibaca
                    </p>
        ...
        <div class="bg-white rounded-2xl shadow-md border border-amber-100 overflow-hidden">
            <div class="px-4 sm:px-6 py-3 border-b border-amber-100 flex items-center justify-between gap-2 bg-[#fffdf7]">
            ...
            @else
                ...
                <div class="md:hidden divide-y divide-amber-100">
                    @foreach ($messages as $message)
                        <div class="px-4 py-4 {{ $message->is_read ? 'bg-white' : 'bg-amber-50/70' }}">
                            {{-- Pengirim + status --}}
                            <div class="flex justify-between items-start mb-2 gap-3">
                                <div class="flex-1">
                                    ...
                                    <a href="mailto:{{ $message->email }}" class="text-xs text-[#c98a00] break-all">
                                        {{ $message->email }}
                                    </a>
                                    @if ($message->telepon)
                                ...
                                @if ($message->is_read)
                                    <span
                                        class="px-2 py-1 rounded-full text-[10px] bg-emerald-50 text-emerald-700 border border-emerald-200 whitespace-nowrap">
                                        Sudah dibaca
                                    </span>
                                @else
                                    <span
                                        class="px-2 py-1 rounded-full text-[10px] bg-amber-50 text-amber-800 border border-amber-300 whitespace-nowrap">
                                        Belum dibaca
                                    </span>
                                @endif
                            </div>

                            {{-- Subjek & pesan singkat --}}
                            ...
                            <div class="mt-3 flex flex-col xs:flex-row gap-2">
                                @if (!$message->is_read)
                                    <form action="{{ route('admin.contact.read', $message->id) }}" method="POST" class="flex-1">
                                        @csrf
                                        <button type="submit"
                                            class="w-full text-[11px] px-3 py-1.5 rounded-lg bg-[#f7e07b] hover:bg-[#f5d547] text-slate-800 font-medium">
                                            Tandai Dibaca
                                        </button>
                                    </form>
                                @endif

                                <button type="button" @click="openDelete('{{ route('admin.contact.destroy', $message->id) }}')"
                                    class="flex-1 text-[11px] px-3 py-1.5 rounded-lg bg-red-50 hover:bg-red-100 text-red-700 border border-red-200 font-medium">
                                    Hapus
                                </button>
                            </div>
                        </div>
                                ...
                                <tr class="{{ $message->is_read ? 'bg-white' : 'bg-amber-50/60' }}">
                                    ...
                                    <td class="px-6 py-4 align-top text-slate-700">
                                        <div>
                                            ...
                                            <a href="mailto:{{ $message->email }}" class="hover:underline text-[#c98a00] break-all">
                                                {{ $message->email }}
                                            </a>
                                        </div>
                                    ...
                                    <td class="px-6 py-4 align-top">
                                        @if ($message->is_read)
                                            <span
                                                class="inline-flex items-center gap-1 px-3 py-1 rounded-full text-[11px] font-semibold bg-emerald-50 text-emerald-700 border border-emerald-200">
                                                <span class="w-1.5 h-1.5 rounded-full bg-emerald-500"></span>
                                                Sudah dibaca
                                            </span>
                                        @else
                                            <span
                                                class="inline-flex items-center gap-1 px-3 py-1 rounded-full text-[11px] font-semibold bg-amber-50 text-amber-800 border border-amber-300">
                ...
                @if (method_exists($messages, 'links'))
                    <div class="px-4 sm:px-6 py-3 border-t border-amber-100 bg-[#fffdf7]">
                        {{ $messages->links() }}
                    </div>
                @endif
            @endif
        ...
        <div x-cloak x-show="showSuccess" x-transition.opacity
            ...
            <div
                class="popup-card bg-gradient-to-br from-[#fff9fb] via-white to-[#fff3d7] rounded-3xl shadow-2xl border border-amber-100 max-w-md w-full mx-4 p-6 relative overflow-hidden">
        ...
        <div x-cloak x-show="confirmDeleteUrl" x-transition.opacity
            class="fixed inset-0 z-40 flex items-center justify-center bg-black/30 backdrop-blur-sm"
            @keydown.escape.window="closeDelete()">

            <div
                class="popup-card bg-gradient-to-br from-white via-[#fff5f5] to-[#ffe4e6] rounded-3xl shadow-2xl border border-rose-100 max-w-md w-full mx-4 p-6 relative overflow-hidden">
                ...
                <div class="relative">
                    <div
                        class="w-14 h-14 rounded-2xl bg-rose-50 flex items-center justify-center mb-4 border border-rose-100">
                        <span class="text-3xl">😢</span>
                    </div>
                    ...
                    <div class="flex items-center gap-2 text-[11px] text-slate-500 mb-4 flex-wrap">
                        <span
                            class="inline-flex items-center gap-1 px-2 py-1 rounded-full bg-white/60 border border-slate-200">
                            <span class="w-1.5 h-1.5 rounded-full bg-rose-400"></span>
                            Tindakan permanen
                        </span>
                        <span
                            class="inline-flex items-center gap-1 px-2 py-1 rounded-full bg-white/60 border border-amber-200">
