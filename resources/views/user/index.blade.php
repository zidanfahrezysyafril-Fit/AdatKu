...
@section('content')
  <div class="space-y-4">
    ...
    <div
      class="bg-white rounded-2xl shadow-sm border border-rose-100 p-4 sm:p-5
          ...
          <div class="border border-rose-100 rounded-2xl px-3 py-3 bg-rose-50/40">
            <div class="flex items-start justify-between gap-2">
              ...
              <div class="text-right space-y-1">
                ...
                @endphp
                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-[11px] {{ $roleClasses }}">
                  {{ ucfirst($role) }}
                </span>
            ...
            <div class="mt-3 flex justify-end gap-2">
              ...
              <a href="{{ route('users.edit', $user->id) }}"
                 class="inline-flex items-center px-3 py-1.5 rounded-lg text-[11px] font-medium
                        text-amber-700 bg-amber-50 border border-amber-100 hover:bg-amber-100">
                Edit
              </a>

              {{-- Hapus (jangan bisa hapus diri sendiri) --}}
              @if (auth()->id() !== $user->id)
                    ...
                    <div class="flex items-center justify-center gap-2">
                      ...
                      @if (auth()->id() !== $user->id)
                        <form action="{{ route('users.destroy', $user->id) }}"
                          ...
                          <button type="submit"
                                  class="inline-flex items-center px-2.5 py-1 rounded-lg
                                         text-[11px] font-medium text-rose-700 bg-rose-50
                                         hover:bg-rose-100">
                            Hapus
                          </button>
                        </form>
