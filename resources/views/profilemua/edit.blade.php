...
<body class="text-slate-800 min-h-screen overflow-x-hidden" x-data="{ open: false }">
    ...
    <main class="pt-16 lg:pl-72"> <!-- penting: beri padding kiri sebesar lebar sidebar -->
        <div class="max-w-5xl mx-auto px-4 py-8">
            ...
            <div class="max-w-5xl mx-auto px-4 py-8">
                {{-- Judul + breadcrumb kecil --}}
                <div class="mb-6">
                    <h1 class="text-2xl md:text-3xl font-bold text-rose-700">Edit Profil MUA</h1>
                    <p class="text-sm text-slate-500 mt-1">Perbarui informasi yang tampil di halaman Pilih MUA.</p>
                </div>
                ...
                @endif
                @if($errors->any())
                    <div class="mb-4 rounded-lg bg-rose-50 text-rose-700 px-4 py-3 text-sm">
                ...
                <form action="{{ route('panelmua.update', $mua->id) }}" method="POST" enctype="multipart/form-data"
                    ...
                    <section class="bg-white rounded-2xl shadow-sm ring-1 ring-slate-200 p-5">
                        ...
                        <div class="flex flex-col md:flex-row items-start gap-5">
                            <div
                                ...
                                <img id="preview"
                                    src="{{ $mua->foto ? asset('storage/' . $mua->foto) : 'https://placehold.co/200x200?text=Foto' }}"
                            ...
                            <div class="flex-1">
                                ...
                                <input type="file" name="foto" accept="image/*" onchange="previewImg(event)"
                                    class="block w-full rounded-lg border border-slate-300 px-3 py-2 focus:ring-2 focus:ring-rose-500 focus:border-rose-500">
                        ...
                        <div class="grid grid-cols-1 gap-5">
                            <div>
                                ...
                                <input type="text" name="nama_usaha" value="{{ old('nama_usaha', $mua->nama_usaha) }}"
                                    class="w-full rounded-lg border border-slate-300 px-3 py-2 focus:ring-2 focus:ring-rose-500 focus:border-rose-500">
                            ...
                            <div>
                                ...
                                <textarea name="deskripsi" rows="4"
                                    class="w-full rounded-lg border border-slate-300 px-3 py-2 focus:ring-2 focus:ring-rose-500 focus:border-rose-500">{{ old('deskripsi', $mua->deskripsi) }}</textarea>
                            </div>
                        </div>
                    </section>
                    ...
                    <div class="flex items-center justify-center gap-3">
                        <a href="{{ route('panelmua.index') }}"
                            class="px-4 py-2 rounded-lg bg-slate-100 text-slate-700 hover:bg-slate-200">Batal</a>
            ...
            @push('scripts')
                <script>
                    function previewImg(e) {
                        ...
                        const r = new FileReader(); r.onload = () => { document.getElementById('preview').src = r.result; };
                        r.readAsDataURL(f);
                    }
                </script>
            @endpush