...
@section('content')
    <div class="w-full px-5 sm:px-6 lg:px-10">
            ...
            <div class="bg-white/95 rounded-3xl ring-1 ring-[#FACC6B]/40 shadow-sm overflow-hidden">
                ...
                <div
                    ...
                    <div class="flex flex-col md:flex-row md:items-end md:justify-between gap-4">
                        ...
                        <a href="{{ route('panelmua.layanan.create') }}" class="inline-flex items-center gap-2 px-5 py-2.5 rounded-2xl font-semibold text-white text-sm
        ...
        : 'text-slate-700 hover:bg-[#FFF3C4]' }}">
                        ...
                        <a href="{{ route('panelmua.layanan.index', ['k' => 'makeup']) }}" class="px-4 py-1.5 rounded-xl text-xs sm:text-sm font-medium
                                   {{ $k === 'makeup'
        ? 'bg-gradient-to-r from-[#FFEB91] to-[#F4B000] text-[#7A4600] shadow-sm'
        : 'text-slate-700 hover:bg-[#FFF3C4]' }}">
                            Makeup
                        ...
                        <a href="{{ route('panelmua.layanan.index', ['k' => 'baju']) }}" class="px-4 py-1.5 rounded-xl text-xs sm:text-sm font-medium
                                   {{ $k === 'baju'
        ? 'bg-gradient-to-r from-[#FFEB91] to-[#F4B000] text-[#7A4600] shadow-sm'
        : 'text-slate-700 hover:bg-[#FFF3C4]' }}">
                            Baju Adat
                        ...
                        <a href="{{ route('panelmua.layanan.index', ['k' => 'pelamin']) }}" class="px-4 py-1.5 rounded-xl text-xs sm:text-sm font-medium
                                   {{ $k === 'pelamin'
        ? 'bg-gradient-to-r from-[#FFEB91] to-[#F4B000] text-[#7A4600] shadow-sm'
        : 'text-slate-700 hover:bg-[#FFF3C4]' }}">
                            Pelamin
                            ...
                            @foreach($items as $item)
                                <div
                                    ...
                                    <div class="relative h-40 sm:h-44 bg-slate-100">
                                        ...
                                        @else
                                            <div
                                                class="w-full h-full flex items-center justify-center text-slate-400 text-xs sm:text-sm">
                                                Tidak ada foto
                                            </div>
                                        @endif

                                        @if($item->kategori)
                                            <span class="absolute top-3 right-3 px-2 py-0.5 text-[10px] font-semibold rounded-full
                    ...
                    @else
                        ...
                        <div
                            ...
                            <div
                                class="h-12 w-12 rounded-full bg-amber-50 flex items-center justify-center text-2xl text-amber-500">
                                ✨
                            </div>
                            <div class="space-y-1">
