...
<head>
  ...
  <style>
    body { background:#fff9f7; }
    .gold-btn{ @apply text-white rounded-xl px-4 py-2 font-medium shadow-md transition; 
    }
  </style>
</head>
  ...
  <div class="min-h-screen flex">
    ...
    <aside :class="openSidebar ? 'w-64' : 'w-16'"
      ...
      <nav class="px-2 space-y-1">
        ...
        <a href="#"
           class="flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-rose-50 @if(request()->routeIs('admin.users.*')) bg-rose-50 text-rose-700 @endif">
          ...
          <div class="flex items-center gap-3 relative">
            ...
            <div x-show="userMenu" @click.outside="userMenu=false"
              ...
              <form method="POST" action="{{ route('logout') }}">
                @csrf
                <button type="submit"
                        class="w-full mt-1 px-3 py-2 rounded-lg text-white
