        ...
        <div class="max-w-6xl mx-auto">
            ...
            @if (session('success') || session('error'))
                <div x-data="{show:true}" x-show="show" x-init="setTimeout(()=>show=false,4000)" x-transition.opacity class="mb-5 flex items-start gap-3 px-4 py-3 rounded-2xl border text-sm shadow-sm
                                                 ...
                                                 @endif">
                    <div class="mt-0.5 inline-flex h-7 w-7 items-center justify-center rounded-full
                                                       @if(session('success'))
                    ...
                    <button @click="show=false" class="ml-auto text-[11px] font-semibold uppercase tracking-wide
            ...
            <div class="bg-white/95 rounded-3xl ring-1 ring-[#FACC6B]/40 shadow-sm overflow-hidden">
                ...
                @else
                                    ...
                                    <tbody class="divide-y divide-amber-50 text-[13.5px]">
                                        @foreach ($pembayaran as $index => $bayar)
                                            ...
                                            <tr class="hover:bg-[#FFF8E8] transition-colors">
                                                ...
                                                <td
                                                    class="px-5 py-4 align-top text-right font-semibold text-amber-600 whitespace-nowrap">
                                                    @if ($pesanan)
                        ...
                        {{-- ========= CARD LIST (MOBILE < SM)=========--}} <div class="sm:hidden px-4 py-4 space-y-3">
                            @foreach ($pembayaran as $index => $bayar)
                                @php
                                    $pesanan = $bayar->pesanan;
                                    $userPesan = $pesanan?->pengguna;
